...
class WaveformSprite extends FlxSprite
{
	...
	public function generate(startPos:Int, endPos:Int)
	{
		...
		for (y in 0...pixels.height)
		{
			...
			pixels.fillRect(new Rectangle((pixels.width / 2) - (w / 2), y, w, 1), 0xFF2600FF);
		}
		pixels.unlock();
	}
	...
	public function generateFlixel(startPos:Float, endPos:Float)
	{
		...
		generate(Math.floor(startPos * multiplicator / 4000 / buffer.bitsPerSample) * buffer.bitsPerSample,
			Math.floor(endPos * multiplicator / 4000 / buffer.bitsPerSample) * buffer.bitsPerSample);
	}
