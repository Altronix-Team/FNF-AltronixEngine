 ...
 */
#if (flixel < "5.0.0")
class EngineSave implements IFlxDestroyable
{
	/**
	...
	public function bind(Name:String, ?Path:String):Bool
	{
		...
		{
			Debug.logError("There was a problem binding to\nthe shared object data from FlxSave.");
			destroy();
			return false;
		}
	...
	function onDone(Result:FlxSaveStatus):Bool
	{
		switch (Result)
		{
			case FlxSaveStatus.PENDING:
				Debug.logWarn("FlxSave is requesting extra storage space.");
			case FlxSaveStatus.ERROR:
				Debug.logError("There was a problem flushing\nthe shared object data from FlxSave.");
			default:
		}
	...
	function checkBinding():Bool
	{
		...
		{
			Debug.logError("You must call FlxSave.bind()\nbefore you can read or write data.");
			return false;
		}
		return true;
	}
...
class EngineSave implements IFlxDestroyable
{
	...
	static function validateAndWarn(str, fieldId:String)
	{
		...
		#if debug
		if (newStr != str)
			Debug.logWarn('FlxSave $fieldId: "$str" contains invalid characters, using "$newStr" instead');
		#end
		return newStr;
	}
	...
	public function bind(name:String, ?path:String):Bool
	{
		...
		{
			Debug.logError('Error:${e.message} name:"$name", path:"$path".');
			destroy();
			return false;
		}
		data = _sharedObject.data;
	...
	function checkStatus():Bool
	{
		...
		{
			case EMPTY:
				Debug.logWarn("You must call FlxSave.bind() before you can read or write data.");
			case ERROR(msg):
				Debug.logError(msg);
			default:
				return true;
