...
class Editor extends TileSubTool
{
	...
	override public function update():Void
	{
		...
		if (_brain.pointerPressed)
		{
			if (_tileBrain.activeTilemap >= 0)
				var b:Bool = _tileBrain.tilemaps[_tileBrain.activeTilemap].setTile(Std.int(_brain.flixelPointer.x / _tileBrain.tileHightligh.width),
					Std.int(_brain.flixelPointer.y / _tileBrain.tileHightligh.height),
					_brain.keyPressed(Keyboard.DELETE) ? 0 : _tileBrain.palette.selectedTile);
		}
	}
	...
	private function drawTileOutline(sprite:FlxSprite):Void
	{
		...
		gfx.drawRect(sprite.x - FlxG.camera.scroll.x, sprite.y - FlxG.camera.scroll.y, sprite.width * 1.0, sprite.height * 1.0);
