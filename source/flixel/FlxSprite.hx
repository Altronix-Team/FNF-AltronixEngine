...
class FlxSprite extends FlxObject
{
	 ...
	 * Rendering variables.
	 */
	public var framesArray(default, set):Array<FlxFramesCollection>;
	...
	public function loadGraphicFromSprite(Sprite:FlxSprite):FlxSprite
	{
        ...
        framesArray = Sprite.framesArray;
	 ...
	 */
	@:noCompletion
	function set_framesArray(Frames:Array<FlxFramesCollection>):Array<FlxFramesCollection>
	{
		if (animation != null)
		{
			animation.destroyAnimations();
		...
		if (Frames != null)
		{
            for (frameObj in Frames)
            {
				graphic = frameObj.parent;
				framesArray.push(frameObj);
				frame = framesArray[0].getByIndex(0);
				numFrames = framesArray[0].numFrames;
				resetHelpers();
				bakedRotationAngle = 0;
				animation.frameIndex = 0;
				graphicLoaded();
