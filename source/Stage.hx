...
class Stage extends MusicBeatState
{
	...
	public function new(daStage:String)
	{
		...
		switch (daStage)
		{
			...
			case "bass" | "bassrift":
				{
					...
					if (daStage == 'bassrift')
					{
						var skyrift:FlxSprite = new FlxSprite(-300, -200).loadGraphic(Paths.image('bass/skyrift'));
						skyrift.antialiasing = true;
						skyrift.scrollFactor.set(0.9, 0.9);
						swagBacks['skyrift'] = skyrift;
						toAdd.push(skyrift);
					}
					else
					{
						var sky:FlxSprite = new FlxSprite(-300, -200).loadGraphic(Paths.image('bass/sky'));
					...
					var houses = new FlxSprite(-270, -250);
					houses.frames = Paths.getSparrowAtlas('bass/houses');
					houses.antialiasing = true;
					houses.scrollFactor.set(0.9, 0.9);
					swagBacks['houses'] = houses;
					toAdd.push(houses);

					if (PlayState.SONG.songId == 'rainglint')
					{
						var floor:FlxSprite = new FlxSprite(-300, 340).loadGraphic(Paths.image('bass/floorpuddles'));
					...
					{
						var floor:FlxSprite = new FlxSprite(-300, 340).loadGraphic(Paths.image('bass/floor'));
						floor.antialiasing = true;
						floor.scrollFactor.set(0.9, 0.9);
						swagBacks['floor'] = floor;
						toAdd.push(floor);
					...
					{
						var hole = new FlxSprite(80, 20);
						hole.frames = Paths.getSparrowAtlas('bass/manifesthole');
						hole.animation.addByPrefix('idle', 'manifest hole', 24, true);
					...
					{
						var rand = FlxG.random.int(2, 6);
						for (i in 1...rand)
						{
							var rift = new FlxSprite(FlxG.random.int(-300, 1920), FlxG.random.int(100, -600));
							rift.frames = Paths.getSparrowAtlas('bass/rift');
							rift.setGraphicSize(Std.int(rift.width * FlxG.random.float(0.5, 1)));
							rift.updateHitbox();
							rift.animation.addByPrefix('idle', 'idle', 6, true);
							rift.animation.addByPrefix('start', 'start', 24, true);
							rift.animation.play('idle');
							rift.antialiasing = true;
							rift.scrollFactor.set(0.9, 0.9);
							swagBacks['rift'] = rift;
							toAdd.push(rift);
						}
					}	
			...
			case 'youtube':
				{
					var bg:FlxSprite = new FlxSprite(100).loadGraphic(Paths.loadImage('bass/youtuberbg', 'shared'));
					bg.scrollFactor.set(0.1, 0.1);
					swagBacks['bg'] = bg;
					toAdd.push(bg);
				}
